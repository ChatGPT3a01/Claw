...
2) **快速驗證（quick_validate.py）**：最小合規檢查；建議保留為相容 wrapper，但核心規則與 `format_check.py` 共用，避免兩邊各自漂移。
...
5) **portability audit**：若有多個 host wrapper，確認它們沒有複製或改寫核心 `SKILL.md`，且 auth / timeout / approval / state path 敘事一致。
...
- 若 skill 支援多個 host，至少保留 `host × workflow` benchmark 與 host-specific regression gates
