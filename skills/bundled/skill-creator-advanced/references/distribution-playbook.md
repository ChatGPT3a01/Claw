...
- 核心 skill folder 應作為單一真實來源；各平台 plugin / manifest / marketplace / config 只做薄包裝
- 先寫 support matrix：primary hosts、secondary hosts、unsupported hosts
- 若 skill 會包工具，先固定共通 tool contract 與 approval 邊界，再做 host-specific adapter
- 快取、登入狀態、安裝產物與其他可變資料不要放在 skill folder
- 若某平台需要大量改寫 workflow，先承認它不是同一個跨工具 skill，不要硬沿用名稱
...
- 若有多個 host wrapper，支援矩陣、授權方式、approval 邊界、state / persistence 敘事是否一致
