...
QUALITY_CHECKLIST_REL = Path("references/quality_checklist.md")
QUALITY_CHECKLIST_REFERENCE = QUALITY_CHECKLIST_REL.as_posix()
QUALITY_CHECKLIST_TOPIC_PATTERNS = {
    "format": (r"\bformat\b", r"\bstructure\b", "格式"),
    "requirements": (r"\brequirements?\b", r"\bpolicy\b", r"\bfunctionality\b", "要求", "規範"),
    "common_errors": (r"\bcommon errors?\b", r"\btroubleshooting\b", "常見錯誤", "錯誤"),
    "final_gate": (r"\bpass\b", r"\bfail\b", r"\bblocked\b", "通過", "未通過", "阻擋"),
}
...
class ValidationProfile:
    check_folder_name: bool = True
    check_root_readme: bool = True
    check_reserved_name: bool = True
    check_frontmatter_angle_brackets: bool = True
    check_name_matches_folder: bool = True
    check_trigger_hint_warning: bool = True
    check_blank_line_warning: bool = True
    check_body_nonempty_warning: bool = True
    check_quality_checklist_topics: bool = True
    check_text_file_format: bool = True
    check_python_compile: bool = True
    check_word_count_warning: bool = True


FULL_PROFILE = ValidationProfile()
MINIMAL_PROFILE = ValidationProfile(
    check_folder_name=False,
    check_root_readme=False,
    check_reserved_name=False,
    check_frontmatter_angle_brackets=False,
    check_name_matches_folder=False,
    check_trigger_hint_warning=False,
    check_blank_line_warning=False,
    check_body_nonempty_warning=False,
    check_quality_checklist_topics=False,
    check_text_file_format=False,
    check_python_compile=False,
    check_word_count_warning=False,
)
...
def lint_skill(
    ...
    *,
    strict: bool = False,
    fix: bool = False,
    profile: ValidationProfile = FULL_PROFILE,
) -> list[Finding]:
    skill_dir = Path(skill_dir).resolve()
    findings: list[Finding] = []

    if not skill_dir.exists():
        return [Finding("ERROR", f"Skill folder not found: {skill_dir}")]
    if not skill_dir.is_dir():
        return [Finding("ERROR", f"Path is not a directory: {skill_dir}")]

    folder_name = skill_dir.name
    if profile.check_folder_name and not is_kebab_case(folder_name):
        findings.append(
            Finding(
                "ERROR",
                "Skill folder name must be kebab-case (lowercase letters/digits separated by single hyphens)",
                skill_dir,
            )
        )

    if profile.check_root_readme and (skill_dir / "README.md").exists():
        findings.append(
            Finding(
                "ERROR",
                "README.md must NOT be inside the skill folder (keep repo-level README outside the skill directory)",
                skill_dir / "README.md",
            )
        )
    ...
    if not skill_md.exists():
        return findings + [
            Finding("ERROR", "SKILL.md not found (must be exactly 'SKILL.md', case-sensitive)", skill_dir)
        ]

    skill_md_text = read_text_safely(skill_md)
    if skill_md_text is None:
        return findings + [Finding("ERROR", "SKILL.md is not valid UTF-8 text", skill_md)]
    ...
    if fm is None:
        return findings + [
            Finding(
                "ERROR",
                "Missing or invalid YAML frontmatter. SKILL.md must start with '---' and include a closing '---' delimiter",
                skill_md,
                1,
            )
        ]
    ...
    if profile.check_frontmatter_angle_brackets and ("<" in frontmatter_text or ">" in frontmatter_text):
    ...
    if frontmatter is not None:
        ...
        if isinstance(desc, str):
            ...
            if not desc_str:
                ...
                if profile.check_trigger_hint_warning and not any(
                    re.search(p, desc_str, flags=re.IGNORECASE) for p in TRIGGER_HINT_PATTERNS
                ):
                    msg = "Description may be missing explicit trigger language (e.g., 'Use when user ...' / '適用於…'). This can cause under-triggering."
                    findings.append(Finding("ERROR" if strict else "WARN", msg, skill_md))

        compatibility = frontmatter.get("compatibility")
        if compatibility:
            if not isinstance(compatibility, str):
                findings.append(Finding("ERROR", f"Compatibility must be a string, got {type(compatibility).__name__}", skill_md))
            elif len(compatibility) > 500:
                findings.append(Finding("ERROR", f"Compatibility is too long ({len(compatibility)} characters). Maximum is 500 characters.", skill_md))

    skill_lines = skill_md_text.splitlines()
    if profile.check_blank_line_warning and fm_end_line < len(skill_lines):
    ...
    if profile.check_body_nonempty_warning and not body_text.strip():
        findings.append(Finding("WARN", "SKILL.md body appears empty (instructions missing)", skill_md))

    if QUALITY_CHECKLIST_REFERENCE in skill_md_text:
        checklist_path = skill_dir / QUALITY_CHECKLIST_REL
        if not checklist_path.exists():
            findings.append(
                Finding(
                    "ERROR",
                    f"SKILL.md references '{QUALITY_CHECKLIST_REFERENCE}' but the file is missing",
                    checklist_path,
                )
            )
        else:
            checklist_text = read_text_safely(checklist_path)
            if checklist_text is None:
                findings.append(Finding("ERROR", f"{QUALITY_CHECKLIST_REFERENCE} is not valid UTF-8 text", checklist_path))
            elif not checklist_text.strip():
                findings.append(Finding("ERROR", f"{QUALITY_CHECKLIST_REFERENCE} is empty", checklist_path))
            elif profile.check_quality_checklist_topics:
                if not contains_any_pattern(checklist_text, QUALITY_CHECKLIST_TOPIC_PATTERNS["format"]):
                    findings.append(Finding("WARN", f"{QUALITY_CHECKLIST_REFERENCE} should cover format/structure checks", checklist_path))
                if not contains_any_pattern(checklist_text, QUALITY_CHECKLIST_TOPIC_PATTERNS["requirements"]):
                    findings.append(Finding("WARN", f"{QUALITY_CHECKLIST_REFERENCE} should cover requirement/policy checks", checklist_path))
                if not contains_any_pattern(checklist_text, QUALITY_CHECKLIST_TOPIC_PATTERNS["common_errors"]):
                    findings.append(Finding("WARN", f"{QUALITY_CHECKLIST_REFERENCE} should cover common error checks", checklist_path))
                if not contains_any_pattern(checklist_text, QUALITY_CHECKLIST_TOPIC_PATTERNS["final_gate"]):
                    findings.append(Finding("WARN", f"{QUALITY_CHECKLIST_REFERENCE} should record a final PASS/FAIL/BLOCKED gate", checklist_path))

    if profile.check_word_count_warning:
    ...
    if profile.check_text_file_format:
        for fp in skill_dir.rglob("*"):
            ...
            if text is None:
                continue
            findings.extend(check_text_file_format(fp, text, fix=fix))

    if profile.check_python_compile:
...
def validate_skill(skill_dir: Path | str, *, profile: ValidationProfile = MINIMAL_PROFILE) -> tuple[bool, str]:
    findings = lint_skill(skill_dir, profile=profile)
    errors = [f for f in findings if f.level == "ERROR"]
    if errors:
        first = sorted(errors, key=lambda x: (str(x.path or ""), x.line or 0, x.message))[0]
...
def main() -> int:
    ...
    findings = lint_skill(args.skill_dir, strict=args.strict, fix=args.fix, profile=FULL_PROFILE)
