...
# ── Font registration ──────────────────────────────────────────────────────────
def _register_cjk_fonts(tokens: dict):
    """Auto-detect and register CJK fonts on Windows/Mac for Chinese support."""
    import platform
    candidates = []
    if platform.system() == "Windows":
        candidates = [
            ("CJK",     os.path.join(os.environ.get("WINDIR", r"C:\Windows"), "Fonts", "msjh.ttc")),
            ("CJK-Bold", os.path.join(os.environ.get("WINDIR", r"C:\Windows"), "Fonts", "msjhbd.ttc")),
        ]
    elif platform.system() == "Darwin":
        candidates = [
            ("CJK",     "/System/Library/Fonts/PingFang.ttc"),
            ("CJK-Bold", "/System/Library/Fonts/PingFang.ttc"),
        ]
    registered = False
    for name, fpath in candidates:
        if os.path.exists(fpath):
            try:
                ...
                pass
    if registered:
        for key in ("font_display_rl", "font_body_rl", "font_body_b_rl"):
            tokens[key] = "CJK" if "Bold" not in key and "display" not in key else "CJK-Bold"
